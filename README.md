...
- 📗 [***YashRajCodes/shard0***](https://github.com/YashRajCodes/shard0) <br/>
  A tool to split a discord's bot shard 0 traffic to multiple workers, uses a single rust worker that listens to shard 0 and pushes to redis streams, and multiple proxies for each worker that listen to the redis streams.
